...
void DWrite::RenderText(wstring text, RECT rect, float size, wstring font, Color color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch , DWRITE_TEXT_ALIGNMENT alignment)
{
    ...
    for (UINT i = 0; i < fontText.size(); i++)
    {
        ...
        {
            //findText = &desc;
            fontText[i].format->SetTextAlignment(alignment);
            findText = &fontText[i];
            break;
        }
    }
    ...
    if (findText == NULL)
    {
        ...
        textDesc.format->SetTextAlignment(alignment);
